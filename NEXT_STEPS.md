...
| F20 | 龙王喷水 | Loop | 喷水向量；方向键/Home 实时调整（Once） |
...
| **Continuous** (State shared) | `Arc<Mutex>` 共享状态 + 1ms 中断循环 | 龙王喷水 | 方向子功能实时修改共享向量，无静态编排 |
...
- 鬼畜走路已迁移到 `RollingPlayer`。龙王喷水（2026-08-14 移植）实测不需要时间轴 — 属共享状态连续控制（`Arc<Mutex>` + 1ms 中断循环，见 `dragon_spin.rs`）
