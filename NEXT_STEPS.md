# Next Steps — GI-Utils v1.2.0

## 当前状态

v1.2.0 — 10 个功能通过 TOML 配置驱动注册。master 48/48 review 全清；gi-utils-gui 分支 2H/12M/15L 全清（含 L12 核心分离）；时间轴调度器 15/15 全清（2026-08-14）；GUI 崩溃自愈两轮 review 全清 + 睡眠唤醒实测通过（2026-08-15）；GUI+托盘系统 ultracode 从零重写 + review 13 条全清 + 实测两轮修复（2026-08-16）。
...
| **Timestamp** (Time based) | 时间轴调度器 | 鬼畜走路、未来钢琴模式 | 每个事件带绝对时间戳，多键可重叠 |
