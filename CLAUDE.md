...
> **定位（2026-08-22 明确）**：个人工具。仓库公开仅作存档与思考记录，
> 不为其他用户/机器做兼容承诺 — 亲和性掩码、TSC 回退频率等均按作者
> 本机（9800X3D 8C16T）写定；功能以单功能使用为主，无并发运行假设
> （2026-08 架构审查在此前提下五项结论全部关闭）。
...
**必须以管理员身份运行**。首次运行自动生成 `profiles/默认.toml`（见「部署」段的 Profile 体系）。

命令行参数：

| 参数 | 行为 |
|---|---|
| `--silent` | **静默启动**：不显示配置窗口，仅托盘图标 + 引擎运行（登录自启用）。实现 = 启动即置 `hidden`（复用「崩溃恢复继承隐藏态」的同一条首帧 SW_HIDE 路径）+ 窗口创建时 `with_active(false)` 不抢焦点；隐藏态首帧请求一次即时重绘，把 eframe「首帧渲染后才显示窗口」造成的可见窗口压到一帧内。第二实例以 `--silent` 启动时**不激活**既有窗口（直接退出，不打断已在运行的实例） |

## CPU 核心分配 (8C16T 9800X3D)

```
物理核 0,4  [0,1 / 8,9]   金银核 — 零后台驻留（CPPC/PBO 实测 VID 最低）
物理核 1,5  [2,3 / 10,11] GAME — 零后台驻留
物理核 2,3  [4-7        ]  GAME + 后台进程驻留（OTHER）
物理核 6    [12,13      ]  GUI (线程级 LOWEST) + 后台驻留
物理核 7    [14,15      ]  TOOL (Engine 输入处理 + 功能线程, REALTIME) + 后台驻留
```

> ⚠️ **核心隔离状态**（2026-08-22 更新，取代 3.2 的"全核 no-op"结论）：
> `GAME_CORES_MASK` 保持全核（缩减游戏核心实测掉帧 + stutter，有意保持）；
> `OTHER_CORES_MASK` 由全核改为 **物理核 2,3,6,7（LP 4-7 + 12-15 = 0xF0F0）**
> 试行 — 金银核 0/4 与相邻 1/5 零后台驻留；后台驻留物理 6,7 无害
> （REALTIME 24 压制普通进程动态带上限 15）。更早"缩减 OTHER 也有奇怪
> 现象"的旧结论由新掩码重新验证中。TOOL/GUI 分区**已落地**（下方进程
> 掩码 + 线程收窄）。「优化游戏」偶数次恢复**有意不降游戏优先级**（3.4）：
...
功能参数（数量/类型在实现期固定）可在 GUI 实时调整并 live-apply —
消灭"调参靠改代码重编"，连点器 v1/v2 同文件复制版的存在理由即此：
...
**设计要点**：`KeyFunction` trait 增加 `parameters() -> &[ParamSpec]`
（ParamSpec = 名字 + 类型/范围/默认值，类型固定几档：f64/u32/bool）；
参数状态为进程级共享（`Arc<...>` 原子或细粒度锁），**功能线程每周期
读取快照** — GUI 修改即刻生效。与现有 live-apply 的关键区分：当前
Save 是全量 stop + 重注册线程，参数改动须走**共享状态直写**，不触发
线程重启、不打断进行中的功能。GUI 面板按已注册绑定渲染参数行
（类型化控件）。首批候选：连点器（interval/hold — v1/v2 归并退役）、
鬼畜走路（节奏）、快速拾取（间隔）。
